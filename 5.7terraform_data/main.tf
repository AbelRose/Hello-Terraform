...
# 注意: 在DataSource中的这个名字和上面的resouces中的名字是不一样的（DataSource中是带S的）
# data 只进行了查询 如果想要输出的话 需要在output中定义一下
data "alicloud_instances" "instance-matrix" {
  name_regex  = "matrix"
}
