...
resource "alicloud_instance" "instance" {
  ...
  # 远程执行命令 remote-exec 创建好instance之后执行的脚本
  provisioner "remote-exec" {
    inline = [
      "mkdir /root/kunkun",
      "echo tieshankao > /root/kunkun/bishaji.txt",
      "yum install -y nginx",
      "systemctl start nginx",
      # 获取实例内部的一些信息
      "curl http://100.100.100.200/latest/meta-data/private-ipv4 > /root/kunkun/private-ipv4.txt"
    ]
    connection {
      type     = "ssh"
      host     = self.public_ip
